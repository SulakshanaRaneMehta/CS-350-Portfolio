...
class CWMachine(StateMachine):
    ...
    ##
	doDot = (
		off.to(dot) | dot.to(off) | dotDashPause.to(dot) | letterPause.to(dot) | wordPause.to(dot)
	)
    ...
    ##
	doDash = (
		off.to(dash) | dash.to(off) | dotDashPause.to(dash) | letterPause.to(dash) | wordPause.to(dash)
	)
    ...
    ##
	doDDP = (
		off.to(dotDashPause) | dotDashPause.to(off) | dot.to(dotDashPause) | dash.to(dotDashPause)
	)
    ...
    ##
	doLP = (
		off.to(letterPause) | letterPause.to(off) | dotDashPause.to(letterPause) | dot.to(letterPause) | dash.to(letterPause)
	)
    ...
    ##
	doWP = (
		 off.to(wordPause) | wordPause.to(off) | letterPause.to(wordPause)
	)
		...
		while not self.endTransmission:
			...
			for word in wordList:
				...
				for char in word:
						...
						if x == ".":
							if self.current_state != self.off:
								if self.current_state == self.dot:
									self.doDot()
								elif self.current_state == self.dash:
									slef.doDash()
								elif self.current_state == self.dotDashPause:
									self.doDDP()
								elif self.current_state == self.letterPause:
									self.doLP()
								elif self.current_state == self.wordPause:
									self.doWP()
								self.doDot()

						else:
							if self.current_state != self.off:
                                				# If not in off, get to off state
								if self.current_state == self.dot:
									self.doDot()  
								elif self.current_state == self.dash:
									self.doDash() 
					...
					if wordCounter < lenWord:
						# Make sure we're in off state
                        			if self.current_state != self.off:
                            				# If not in off, get to off state
                            				if self.current_state == self.dotDashPause:
                                				self.doDDP()  
				...
				if wordsCounter < lenWords:
					...
					if self.current_state != self.off:
                        			# If not in off, get to off state
						if self.current_state == self.letterPause:
							self.doLP()  
